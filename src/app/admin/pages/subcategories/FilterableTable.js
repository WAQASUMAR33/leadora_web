...
const FilterableTable = ({ subcategories = [], fetchSubcategories }) => {
  ...
  const handleAddNewItem = async () => {
    console.log('Sending slug for update:', newSubcategory.slug);
    setIsLoading(true);
    try {
      ...
      console.log('Slug being sent to the API:', updatedSlug);
  ...
  const handleEditItem = (item) => {
    console.log('Editing item with slug:', item.slug);
    setNewSubcategory({
      ...item,
      slug: item.slug,
      image: null,
          ...
          <Table stickyHeader>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 'bold', bgcolor: 'grey.50' }}>Category</TableCell>
              ...
              {Array.isArray(paginatedData) && paginatedData.length > 0 ? (
                paginatedData.map((item, index) => (
                  <TableRow key={item.id} sx={{ bgcolor: index % 2 === 0 ? 'white' : 'grey.50' }}>
                    ...
                    <TableCell sx={{ color: 'grey.500' }}>{item.category?.name || 'N/A'}</TableCell>
                    <TableCell sx={{ color: 'grey.500' }}>
                ...
                <TableRow>
                  <TableCell colSpan={8} sx={{ textAlign: 'center', color: 'grey.500' }}>
                    No data available
                  </TableCell>
                </TableRow>
              )}
            ...
            {newSubcategory.imageUrl && (
              <Grid item xs={12}>
                ...
                <Image
                  ...
                  style={{ width: '8rem', height: '8rem', objectFit: 'cover', marginBottom: '8px' }}
                />
              </Grid>
            )}
