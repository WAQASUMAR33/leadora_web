...
const ModernProgress = styled(Box)(({ theme }) => ({
  ...
  '&:before': {
    ...
    width: '100%',
    height: '100%',
    background: 'linear-gradient(90deg, #F25C2C, #c46cf7, #FB923C, #FBBF24, #F25C2C)', // Orange-based looping gradient
...
const getStatusStyles = (status) => {
  ...
  if (s.includes('confirmed')) return { bgcolor: '#FFF7ED', color: '#EA580C', border: '1px solid #FFEDD5' };
  if (s.includes('paid')) return { bgcolor: '#FFF7ED', color: '#F25C2C', border: '1px solid #FFEDD5' };
  if (s.includes('shipped')) return { bgcolor: '#E0E7FF', color: '#4F46E5', border: '1px solid #C7D2FE' };
  if (s.includes('completed')) return { bgcolor: '#D1FAE5', color: '#059669', border: '1px solid #6EE7B7' };
  if (s.includes('cancelled')) return { bgcolor: '#FEE2E2', color: '#DC2626', border: '1px solid #FCA5A5' };
                  ...
                  >
                    ...
                    <TableCell sx={{ fontSize: '0.8rem' }}>
                      {item.orderItems && item.orderItems.length > 0 && item.orderItems[0].product?.images?.[0] ? (
                        ...
                        <Box sx={{ width: 36, height: 36, bgcolor: '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <DialogContent sx={{ px: 4, py: 2 }}>
          ...
          <Box>
            ...
            {newItem.orderItems.length === 0 ? (
              <Box sx={{ p: 4, bgcolor: '#F9FAFB', border: '2px dashed #E5E7EB', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
